...
export default function LoginPage() {
  ...
  async function handleSignin(e: React.FormEvent) {
    ...
    if (error) { setSiError(error.message.toLowerCase().includes('invalid login') ? 'Wrong email or password. Double-check and try again.' : error.message); setSiLoading(false); return }
      ...
      <div className="auth-left">
        ...
        <div className="auth-quote">
          ...
          <p className="auth-body">
            Sign in to access every Meridian product you have paid for — the dictionary, the course, the stock analyser, and the business tracker. All in one place.
          </p>
        </div>
        ...
        <div className="auth-wod">
          ...
          <div className="auth-wod-def">How quickly you can turn what you own into cash when you actually need it.</div>
          ...
          <div className="auth-wod-reality">
            ...
            <div className="auth-wod-reality-body">
              Before you put money anywhere, ask one question: <strong>&ldquo;If something happens tonight, can I get this money back in seven days?&rdquo;</strong> If the answer is no, that investment is illiquid. Keep at least three months of your expenses somewhere you can reach overnight. Invest the rest.
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="auth-card">
          {suSuccess ? (
            <div style={{ textAlign: 'center' }}>
              <div style={{ width: 56, height: 56, background: 'rgba(10,61,34,0.08)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', color: 'var(--forest)', fontSize: 22 }}>✓</div>
              <h2 className="auth-heading" style={{ textAlign: 'center', fontSize: 36, marginBottom: 12 }}>Check your email.</h2>
              <p className="auth-sub" style={{ textAlign: 'center' }}>We sent a confirmation link to <strong style={{ color: 'var(--forest)' }}>{suEmail}</strong>. Click it to activate your account, then come back to sign in.</p>
              <button onClick={() => setActiveTab('signin')} className="btn btn-primary" style={{ marginTop: 8 }}>Go to sign in →</button>
            </div>
          ) : (
            <>
                ...
                <form onSubmit={handleSignin}>
                  ...
                  <div className="form-group">
                    ...
                    <div className="form-input-wrap">
                      ...
                      <button type="button" className="form-input-eye" onClick={() => setSiShowPwd(p => !p)} tabIndex={-1}>{siShowPwd ? '🙈' : '👁'}</button>
                    </div>
                  </div>
                  {forgotMsg && <div className="sys-msg sys-ok" style={{ marginBottom: 12 }}>{forgotMsg}</div>}
                  {siError   && <div className="sys-msg sys-error">{siError}</div>}
                  <div style={{ marginTop: 24 }}>
                    <button type="submit" className={`btn btn-primary${siLoading ? ' btn-loading' : ''}`} disabled={siLoading}>{siLoading ? '' : 'Sign in to my dashboard →'}</button>
                  </div>
                  <div className="auth-link-row">No account yet? <a onClick={() => setActiveTab('signup')} style={{ cursor: 'pointer' }}>Create one free</a></div>
                </form>
              )}
              ...
              {activeTab === 'signup' && (
                <form onSubmit={handleSignup}>
                  ...
                  <div className="form-group">
                    ...
                    <div className="form-input-wrap">
                      <input id="su-pwd" type={suShowPwd ? 'text' : 'password'} className="form-input" placeholder="At least 8 characters" value={suPwd} onChange={e => setSuPwd(e.target.value)} required minLength={8} autoComplete="new-password"/>
                      <button type="button" className="form-input-eye" onClick={() => setSuShowPwd(p => !p)} tabIndex={-1}>{suShowPwd ? '🙈' : '👁'}</button>
                    </div>
                  </div>
                  {suError && <div className="sys-msg sys-error">{suError}</div>}
                  <div style={{ marginTop: 24 }}>
                    <button type="submit" className={`btn btn-primary${suLoading ? ' btn-loading' : ''}`} disabled={suLoading}>{suLoading ? '' : 'Create my account →'}</button>
                  </div>
                  <div className="auth-link-row">Already have an account? <a onClick={() => setActiveTab('signin')} style={{ cursor: 'pointer' }}>Sign in</a></div>
                </form>
              )}

              <div className="auth-legal">By continuing you agree to Meridian&apos;s Terms of Use. Meridian does not provide financial advice.</div>
            </>
          )}
        </div>
      </div>
